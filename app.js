...
app.all('/', function(req, res) {
  ...
  if (req.method == 'GET') {
    ...
    db.query('SELECT * FROM users WHERE login = ?', [req.body.login], function(
      err,
      rows
    ) {
...
io.sockets.on('connection', function(socket) {
  ...
  socket.on('playerIsDead', function(killerLsid, room) {
    ...
    if (killerName) {
      ...
      db.query(
        'UPDATE users SET kills = kills+1 WHERE LOGIN = ?',
        [killerName],
        function(err, result) {
          if (err) console.log(err);
        }
      ...
      db.query(
        'UPDATE users SET deaths = deaths+1 WHERE LOGIN = ?',
        [deadPlayerName],
        function(err, result) {
          if (err) console.log(err);
        }
